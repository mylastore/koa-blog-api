...
class UserController {
  ...
  async register(ctx) {
    ...
    await jwt.verify(token, userActivationSecret, async function (err, decoded) {
      ...
      const obj = {name, email, password, avatar, username, emailVerificationToken: undefined, emailVerified: true}
      ...
      try {
        ...
        if (result) {
          ...
          await _data.read('settings', settingId, function (err, checkData) {
  ...
  async googleLogin(ctx){
    ...
    try{
      const res = await client.verifyIdToken({idToken, audience: googleId})
      if(!res){
      ...
      if(user){
        ...
        await ctx.cookies.set('token', authUser.token, {expiresIn: sessionExpiration})
        ctx.body = authUser
      } else {
        ...
        const user = new User({name, email, username, password, avatar})
