...
class UserController {
  async accountActivation(ctx) {
    ...
    try {
      ...
      await accountActivationEmail(ctx, email, token)
      return (ctx.body = {
        status: 200,
  ...
  async createQuote(ctx) {
    ...
    try {
      ...
      await sendQuoteEmail(name, email, tel, site, msg, reason)
      ctx.body = 'Success!'
    } catch (err) {
      ctx.throw(err)
    }
  ...
  async adminGetUser(ctx) {
    try {
      return ctx.body = await User.findById({_id: ctx.params.id}).select({
        settings: 1,
        profile: 1,
        email: 1,
        role: 1,
  ...
  async getStats(ctx) {
    try {
      return ctx.body = await User.count({})
    } catch (err) {
      ctx.throw(422, err)
    }
  }
